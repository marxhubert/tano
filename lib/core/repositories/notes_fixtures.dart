...
/// Called only by the explicit developer reset, never at startup.
TanoFixtures buildFixtures({required bool canLock, DateTime? now}) {
  final Random random = Random(42);
  final DateTime baseDate = now ?? DateTime.now();
  ...
  final List<Folder> folders = <Folder>[
    for (int i = 0; i < 5; i++)
      Folder(
        ...
        isLocked: canLock && i < 4 && folderLocks[i],
      ),
  ];
