...
Note _note(String id, {String? title, String? date, bool important = false, String? category}) {
    return Note(
        id: id,
        title: title ?? 'Note $id',
        content: 'Contenu $id',
...
void main() {
  group('HomeViewModel', () {
    ...
    test('toggleFavorite inverse le statut et persiste', () async {
      final repository = _InMemoryNotesRepository(<Note>[_note('1')]);
      final vm = HomeViewModel(repository: repository, initialNotes: repository.notes);
    ...
    test('la sélection multiple supprime toutes les notes sélectionnées', () async {
    ...
    test('applyNoteAction remplace la note modifiée', () async {
      ...
      final vm = HomeViewModel(
        repository: repository,
        initialNotes: <Note>[_note('1', title: 'Avant'), _note('2')],
      );

      await vm.applyNoteAction(
        add: false,
        index: 0,
        action: NoteAction(action: 'Save', note: _note('1', title: 'Après')),
      );
