...
void main() {
  group('EditNoteViewModel', () {
    test('buildNote utilise le titre quand il est fourni', () {
      final vm = EditNoteViewModel(repository: _InMemoryNotesRepository(), add: true);
    ...
    test('isDirty est vrai quand le contenu change', () {
      final note = Note(id: '1', title: 'Titre', content: 'Contenu', date: '2026-08-01 10:00:00.000');
      final vm = EditNoteViewModel(
        repository: _InMemoryNotesRepository(),
        add: false,
        initialNote: note,
    ...
    test('persistSavedNote ajoute une nouvelle note', () async {
      final repository = _InMemoryNotesRepository(<Note>[
        Note(id: '1', title: 'Existante', content: 'x', date: '2026-08-01 10:00:00.000'),
      ]);
      final vm = EditNoteViewModel(repository: repository, add: true);

      await vm.persistSavedNote(Note(id: '42', title: 'Nouvelle', content: 'y', date: '2026-08-02 10:00:00.000'));
    ...
    test('persistSavedNote remplace la note existante par son id', () async {
      final repository = _InMemoryNotesRepository(<Note>[
        Note(id: '1', title: 'Avant', content: 'x', date: '2026-08-01 10:00:00.000'),
        Note(id: '2', title: 'Intacte', content: 'y', date: '2026-08-02 10:00:00.000'),
      ]);
      final vm = EditNoteViewModel(repository: repository, add: false, initialNote: repository.notes.first);

      await vm.persistSavedNote(Note(id: '1', title: 'Après', content: 'z', date: '2026-08-01 10:00:00.000'));
